...
public class Character : Unit
{
  ...
  public void CharacterInfo()
  {
    ...
    Console.WriteLine($"Особый удар способности: {_skill.SpecialAttackDescription}");
  ...
  public override SpecialAttackDamageInfo SpecialAttack(
    IEnumerable<Unit> units,
    int selectedUnitIndex,
    Character character
  )
  {
    var damageInformationText = "";
    var totalDamage = 0f;

    var damage = _skill.SpecialAttack(units, selectedUnitIndex, GetDamageCalculationParameters(), character);
    foreach (Enemy unit in damage.TakenDamageEnemyList)
    {
      var damageReceived = unit.TakeDamage(new Damage
        { DamageValue = damage.DamageValue, DamageType = damage.DamageType });
      damageInformationText += $"Вы нанесли {damageReceived} врагу {unit.Name}." +
                               $" Им было заблокировано {damage.DamageValue - damageReceived}\n";

      totalDamage += damageReceived;
    }

    return new SpecialAttackDamageInfo { DamageValue = totalDamage, DamageInformationText = damageInformationText };
  }
  ...
  public bool IsSpecialAttackIsUsed()
  {
    return _skill.SpecialAttackIsUsed;
  }

  public void ChangeSpecialAttackIsUsed(bool value)
  {
    _skill.SpecialAttackIsUsed = value;
  }
}